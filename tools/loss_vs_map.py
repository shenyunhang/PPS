...
def get_mAP(log_path):
    ...
    for line in log_file.readlines():
        line = line.strip()
        ma = re.search('Single Query:[ ]+\[mAP: [0-9]+([.][0-9]+)?\%\]', line)
        if ma is None:
            continue
        # print(ma)
        # print(ma.group())
    ...
    return mAP_values[0], mAP_values[1:][::-1]


def draw():
    log_path = sys.argv[1]
    loss_values, iter_values, snapshot_value = get_loss(log_path)
    log_path = sys.argv[2]
    map_final, mAP_values = get_mAP(log_path)

    mAP_epoch_values = range(1,
    ...
    major_ticks = np.arange(1, mAP_epoch_values[-1], 5)
    # minor_ticks = np.arange(0, mAP_epoch_values[-1], 1)
    ax1.set_xticks(major_ticks)
    # ax1.set_xticks(minor_ticks, minor=True)
    ax1.grid(which='both')

    major_ticks = np.arange(40, 90, 5)
    ...
    ax1.set_xlim([0, mAP_epoch_values[-1]])
    ax1.set_ylim([0, 100])
    plt.savefig(os.path.join(output_dir, uuid + '_plot_100.png'), dpi=1200)

    # set the limits
    ax1.set_xlim([0, mAP_epoch_values[-1]])
    ax1.set_ylim([0, 25])
    plt.savefig(os.path.join(output_dir, uuid + '_plot_25.png'), dpi=1200)
