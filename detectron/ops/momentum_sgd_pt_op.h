...
void momentum_sgd_pt_update(const int N, const float* g, const float* m,
                            float* ng, float* nm, const float* lr,
                            const float momentum, const bool nesterov,
                            float* param, Context* /*context*/) {
  const float LR = lr[0];
  for (auto i = 0; i < N; ++i) {
    if (!nesterov) {
...
class MomentumSGDPTOp final : public Operator<Context> {
 public:
  USE_OPERATOR_CONTEXT_FUNCTIONS;
  MomentumSGDPTOp(const OperatorDef& operator_def, Workspace* ws)
  ...
  bool RunOnDevice() override {
    ...
    momentum_sgd_pt_update<Context>(
        Input(GRAD).numel(), Input(GRAD).template data<T>(),
        Input(MOMENTUM).template data<T>(),
        Output(OUTPUT_GRAD)->template mutable_data<T>(),
        Output(OUTPUT_MOMENTUM)->template mutable_data<T>(),
        Input(LR).template data<T>(), momentum_, nesterov_, NULL, &context_);
    return true;
  }
...
template <typename T, class Context>
class MomentumSGDUpdatePTOp final : public Operator<Context> {
 public:
  USE_OPERATOR_CONTEXT_FUNCTIONS;
  MomentumSGDUpdatePTOp(const OperatorDef& operator_def, Workspace* ws)
      : Operator<Context>(operator_def, ws),
        momentum_(this->template GetSingleArgument<T>("momentum", 0.0)),
        nesterov_(this->template GetSingleArgument<int>("nesterov", 0)) {}
  ...
  bool RunOnDevice() override {
    ...
    momentum_sgd_pt_update<Context>(
        ...
        Output(OUTPUT_GRAD)->template mutable_data<T>(),
        Output(OUTPUT_MOMENTUM)->template mutable_data<T>(),
        Input(LR).template data<T>(), momentum_, nesterov_,
...
template <typename T, class Context>
class SparseMomentumSGDUpdatePTOp final : public Operator<Context> {
 public:
  USE_OPERATOR_CONTEXT_FUNCTIONS;
  SparseMomentumSGDUpdatePTOp(const OperatorDef& operator_def, Workspace* ws)
      : Operator<Context>(operator_def, ws),
        momentum_(this->template GetSingleArgument<T>("momentum", 0.0)),
        nesterov_(this->template GetSingleArgument<int>("nesterov", 0)) {}
  ...
  bool RunOnDevice() override {
    ...
    return DispatchHelper<TensorTypes<int32_t, int64_t>>::call(this,
                                                               Input(INDICES));
  }
  ...
  bool DoRunWithType() {
    ...
    for (auto i = 0; i < n; ++i) {
      ...
      momentum_sgd_pt_update<Context>(
          block_size, gradIn + offsetI, momentumIn + offsetIdx,
          gradOut + offsetI, momentumOut + offsetIdx, lr, momentum_, nesterov_,
          paramOut + offsetIdx, &context_);
    }
    return true;
  }
