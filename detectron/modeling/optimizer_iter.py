...
def add_single_gpu_param_update_ops(model, gpu_id):
    ...
    for param in model.TrainableParams(gpu_id=gpu_id):
        ...
        param_acmgrad = model.param_init_net.ConstantFill(
            [param], param + '_acmgrad', value=0.0
        )
        ...
        if ('bpm' in str(param) or 'apm' in str(param) or 'crm' in str(param) or 'ekc' in str(param) or 'pps' in str(param) or 'youtu' in str(param)) and 'fc' in str(param): 
            ...
            model.net.ACMWeightDecayMomentumSGDUpdate(
                [param_grad, param_momentum, lr_new_fc, param, param_acmgrad],
                [param_grad, param_momentum, param, param_acmgrad],
                momentum=cfg.SOLVER.MOMENTUM,
                iter_size=cfg.REID.ITER_SIZE,
                gpu_num=cfg.NUM_GPUS,
                lr_mult=lr_mult,
            ...
            # Update param_grad and param_momentum in place
            model.net.ACMWeightDecayMomentumSGDUpdate(
                [param_grad, param_momentum, lr_new_param, param, param_acmgrad],
                [param_grad, param_momentum, param, param_acmgrad],
                momentum=cfg.SOLVER.MOMENTUM,
                iter_size=cfg.REID.ITER_SIZE,
                gpu_num=cfg.NUM_GPUS,
        ...
        # Update param_grad and param_momentum in place
        model.net.ACMWeightDecayMomentumSGDUpdate(
            [param_grad, param_momentum, lr, param, param_acmgrad],
            [param_grad, param_momentum, param, param_acmgrad],
            momentum=cfg.SOLVER.MOMENTUM,
            iter_size=cfg.REID.ITER_SIZE,
            gpu_num=cfg.NUM_GPUS,
