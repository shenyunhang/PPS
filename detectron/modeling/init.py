...
def uniform_(a=0, b=1):
    return ('UniformFill', {'min': a, 'max': b})
...
def _calculate_fan_in_and_fan_out(tensor_size):
    ...
    if dimensions == 2:  # Linear
        ...
        fan_in = tensor_size[1]
        fan_out = tensor_size[0]
    else:
...
def kaiming_uniform_(tensor_size, a=0, mode='fan_in', nonlinearity='leaky_relu'):
    r"""Fills the input `Tensor` with values according to the method
    ...
    fan = _calculate_correct_fan(tensor_size, mode)
    gain = calculate_gain(nonlinearity, a)
    std = gain / math.sqrt(fan)
    bound = math.sqrt(3.0) * std  # Calculate uniform bounds from standard deviation
    # with torch.no_grad():
        # return tensor.uniform_(-bound, bound)
    return ('UniformFill', {'min': -bound, 'max': bound})


def bias_init_(tensor_size):
    fan_in, _ = _calculate_fan_in_and_fan_out(tensor_size)
