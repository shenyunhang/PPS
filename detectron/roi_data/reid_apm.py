...
def add_reid_blobs(blobs, im_scales, im_crops, roidb):
...
def _sample_rois(roidb, im_scale, im_crop, batch_idx):
    ...
    if rois_this_image < rois_per_image:
        new_boxes = np.array([0, 0, 1000, 1000], dtype=sampled_boxes.dtype)
        new_boxes = np.tile(new_boxes, [rois_per_image - rois_this_image, 1])
        new_score = np.zeros(
            (rois_per_image - rois_this_image, 1), dtype=sampled_scores.dtype)
        sampled_boxes = np.vstack((sampled_boxes, new_boxes))
        sampled_scores = np.vstack((sampled_scores, new_score))
...
def random_crop(im):
    ...
    h_start = np.random.randint(0, im.shape[0] - crop_h[1])
    w_start = np.random.randint(0, im.shape[1] - crop_w[0])
...
def random_erasing(img):
    ...
    sh = cfg.REID.SH
    r1 = cfg.REID.R1
    if random_erasing_prob == 0 or np.random.uniform > random_erasing_prob:
        return img

    for attempt in range(100):
        area = img.shape[0] * img.shape[1]
        target_area = np.random.uniform(sl, sh) * area
        aspect_ratio = np.random.uniform(1.5, 1 / self.r1)
        ...
        if w < img.shape[1] and h < img.shape[0]:
            ...
            if img.shape[2] == 3:
                img[x1:x1 + h, y1:y1 + w, 0] = cfg.PIXEL_MEANS[0]
                img[x1:x1 + h, y1:y1 + w, 1] = cfg.PIXEL_MEANS[1]
                img[x1:x1 + h, y1:y1 + w, 2] = cfg.PIXEL_MEANS[2]
            else:
                img[x1:x1 + h, y1:y1 + w, 0] = cfg.PIXEL_MEANS[0]
            return img
    return img
...
def _project_im_rois(im_rois, im_scale_factor, im_crop):
    ...
    rois = (im_rois - crop)
    rois[:, 0] = rois[:, 0] * im_scale_factor[0]
    rois[:, 1] = rois[:, 1] * im_scale_factor[1]
    rois[:, 2] = rois[:, 2] * im_scale_factor[0]
    rois[:, 3] = rois[:, 3] * im_scale_factor[1]
